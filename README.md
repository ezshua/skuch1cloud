...
- [ ] **Гибкая выгрузка медиа:** Реализовать переключение режима отправки между `Media` (фото/видео сразу в чате) и `Document` (файлом без сжатия) на основе расширения или настроек пользователя.
