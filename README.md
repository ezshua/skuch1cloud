...
*   **Защита от некорректных ссылок** — бот проверяет протоколы (только HTTP/HTTPS) и тип контента, блокируя попытки скачать веб-страницы или каталоги вместо файлов.
...
- `LOG_FILE_SIZE_LIMIT` — лимит размера файла журнала действий (`action_log.json`). При превышении лимита (по умолчанию: 5 МБ) бот автоматически удаляет 2% самых старых записей, обеспечивая ротацию логов без остановки системы.
- `ADMIN_ACCOUNT_ID` — цифровой ID вашего Telegram аккаунта для доступа к командам `/report`. Чтобы узнать свой ID, напишите боту `@userinfobot` или `@getmyid_bot`.
