...
| **\_\_main\_\_.py** | Точка входа модуля | Альтернативный запуск проекта как модуля (`python -m src`) |
...
*   **Обработка дубликатов** — если файл с таким именем уже существует, бот сохранит новый файл под уникальным именем, не перезаписывая старый.
...
*   **Надежность и самоочистка** — автоматическое удаление временных файлов (`.download`, `.tmp`) при старте системы, по команде `/start` или немедленно при обрыве соединения/тайм-ауте.
*   **Защита от некорректных ссылок** — бот проверяет протоколы (только HTTP/HTTPS), тип контента, перенаправления и блокирует скачивание с локальных/служебных сетевых адресов.
...
| `/mediaon` | Включить мультимедийный режим: фото и видео отправляются как медиа (видны прямо в чате). |
| `/mediaoff` | Выключить мультимедийный режим (по умолчанию): все файлы отправляются как документы. |
| `/preview [страница]` | Альбом превью фото и видео с пагинацией (по 10 на страницу). |
| `/report` | (Админ) Получить отчет об активности пользователей за последние 24 часа. |
| `/report daily` | (Админ) Включить/выключить автоматическую рассылку отчетов в полночь. |
| `/report users` | (Админ) Сводка по всем пользователям: кол-во файлов и объем данных. |
...
*   **Текстовое сообщение со ссылкой HTTP/HTTPS** — бот скачает файл по ссылке, если адрес проходит сетевые проверки безопасности.
...
Основной поддерживаемый способ установки зависимостей для текущей структуры проекта — `requirements.txt`.
...
- `MAX_FILE_SIZE` — положительное целое число; максимальный размер скачиваемого файла в байтах (по умолчанию: `20971520`, то есть 20 МБ)
- `BOT_DATA_PATH_SIZE` — положительное целое число; общий лимит дискового пространства для сохранения файлов в байтах (по умолчанию: `524288000`, то есть 500 МБ)
- `LOG_FILE_SIZE_LIMIT` — положительное целое число; лимит размера файла журнала действий (`action_log.json`). При превышении лимита бот автоматически оставляет последние 80% записей и удаляет примерно 20% самых старых.
- `ADMIN_ACCOUNT_ID` — положительное целое число; цифровой ID вашего Telegram аккаунта для доступа к командам `/report`. Если переменная не задана, админ-команды и уведомления администратора отключены. Чтобы узнать свой ID, напишите боту `@userinfobot` или `@getmyid_bot`.

### Защита URL-загрузок

URL-загрузки включают базовую SSRF-защиту: бот не скачивает файлы с `localhost`, приватных сетей, link-local, multicast, reserved и unspecified IP-адресов. Проверяется исходный URL и каждое перенаправление.

Если бот должен скачивать файлы из локальной сети, защиту можно отключить в `src/url_handler.py`, установив:
...
- [x] **Гибкая выгрузка медиа:** Реализовать переключение режима отправки между `Media` (фото/видео сразу в чате) и `Document` (файлом без сжатия) на основе расширения или настроек пользователя. *(Реализовано: команды `/mediaon` и `/mediaoff`, настройка сохраняется в `user_settings.json`.)*
- [ ] **Конвертация TGS-стикеров:** Анимированные стикеры (Lottie/TGS) конвертировать в GIF/MP4 для воспроизведения при выдаче. Требует библиотеку `rlottie` или `lottie`.
- [ ] **Массовая выгрузка:** Добавить поддержку специального символа `*.*` для выгрузки всех сохраненных файлов пользователя.
