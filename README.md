...
Это происходит потому, что команда `sudo -u` выполняет команду от имени другого пользователя, но **не создает для него полноценную сессию входа**. В результате переменные окружения, необходимые для связи с пользовательскими службами (`DBUS_SESSION_BUS_ADDRESS` и `XDG_RUNTIME_DIR`), не устанавливаются.
